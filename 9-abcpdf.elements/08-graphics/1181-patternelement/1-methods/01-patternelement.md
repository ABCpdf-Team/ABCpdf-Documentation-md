...
```csharp
PatternElement()
PatternElement(Atom atom, IndirectObject host)
PatternElement(IndirectObject obj)
PatternElement(Element relation, CreationOptions options)
```
...
```
PatternElement()
PatternElement(atom As Atom, host As IndirectObject)
PatternElement(obj As IndirectObject)
PatternElement(relation As Element, options As CreationOptions)
```
