...
| **[C#]** ```csharp OpiElement() OpiElement(Atom atom, IndirectObject host) OpiElement(IndirectObject obj) OpiElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` OpiElement() OpiElement(atom As Atom, host As IndirectObject) OpiElement(obj As IndirectObject) OpiElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
