...
|  |  | AtomicArrayElement Function |  |  | 
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new AtomicArrayElement. |  |  | 
...
| **[C#]** ```csharp AtomicArrayElement() AtomicArrayElement(Atom atom, IndirectObject host) AtomicArrayElement(IndirectObject obj) AtomicArrayElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` AtomicArrayElement() AtomicArrayElement(atom As Atom, host As IndirectObject) AtomicArrayElement(obj As IndirectObject) AtomicArrayElement(relation As Element, options As CreationOptions) ``` `ArgumentException: Thrown if the Atom provided is not an ArrayAtom or a RefAtom pointing to an ArrayAtom.` |  |  | 
| --- | --- | --- |
