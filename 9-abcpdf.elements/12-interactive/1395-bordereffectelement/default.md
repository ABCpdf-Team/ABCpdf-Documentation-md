---
title: "default"
css: "abcpdf-docs.css"
---

# BorderEffectElement Class

This class represents the border effect dictionary. This is definitively detailed in:.

[The ISO PDF Specification, ISO 32000-1:2008 PDF 1.7; Table: 167, page 387.](https://opensource.adobe.com/dc-acrobat-sdk-docs/standards/pdfstandards/pdf/PDF32000_2008.pdf#page=395)

[The ISO PDF Specification, ISO 32000-2:2017 PDF 2.0; Table: 169, page 470.](https://www.iso.org/standard/63534.md)

``` System.Object WebSupergoo.ABCpdf13.Elements.Element WebSupergoo.ABCpdf13.Elements.BorderEffectElement ```

## Method Description BorderEffectElement Create a new BorderEffectElement. inherited methods... &nbsp;

## Property Description EntryS Represents the "S" entry of the border effect dictionary object. EntryI Represents the "I" entry of the border effect dictionary object. inherited properties...