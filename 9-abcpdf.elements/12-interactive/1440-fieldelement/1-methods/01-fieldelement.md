...
| **[C#]** ```csharp FieldElement() FieldElement(Atom atom, IndirectObject host) FieldElement(IndirectObject obj) FieldElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` FieldElement() FieldElement(atom As Atom, host As IndirectObject) FieldElement(obj As IndirectObject) FieldElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
