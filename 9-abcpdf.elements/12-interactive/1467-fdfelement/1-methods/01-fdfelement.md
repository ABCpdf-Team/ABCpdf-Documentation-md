...
```csharp
FDFElement()
FDFElement(Atom atom, IndirectObject host)
FDFElement(IndirectObject obj)
FDFElement(Element relation, CreationOptions options)
```
...
```
FDFElement()
FDFElement(atom As Atom, host As IndirectObject)
FDFElement(obj As IndirectObject)
FDFElement(relation As Element, options As CreationOptions)
```
