...
```csharp
CuePointElement()
CuePointElement(Atom atom, IndirectObject host)
CuePointElement(IndirectObject obj)
CuePointElement(Element relation, CreationOptions options)
```
...
```
CuePointElement()
CuePointElement(atom As Atom, host As IndirectObject)
CuePointElement(obj As IndirectObject)
CuePointElement(relation As Element, options As CreationOptions)
```
