...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new FileAttachmentAnnotationElement. |  |  | 
...
| **[C#]** ```csharp FileAttachmentAnnotationElement() FileAttachmentAnnotationElement(Atom atom, IndirectObject host) FileAttachmentAnnotationElement(IndirectObject obj) FileAttachmentAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` FileAttachmentAnnotationElement() FileAttachmentAnnotationElement(atom As Atom, host As IndirectObject) FileAttachmentAnnotationElement(obj As IndirectObject) FileAttachmentAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 1656 in the Annotations example project. [C#] ```csharp FileAttachmentAnnotationElement annot = new FileAttachmentAnnotationElement(AnnotationElement.Object); annot.EntrySubtype = "FileAttachment"; annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(new XRect(rect)), annot.Host); FullFileSpecificationElement fileSpec = new FullFileSpecificationElement(annot); fileSpec.EntryType = "Filespec"; FileInfo fileInfo = new FileInfo(filePath); fileSpec.EntryF = fileInfo.Name; fileSpec.EntryUF = fileInfo.Name; fileSpec.EntryEF = new EmbeddedFileSetElement(annot); fileSpec.EntryEF.EntryF = EmbedFileStream(fileInfo); annot.EntryFS = fileSpec; AnnotationElement = annot; ``` |  |  | 
| --- | --- | --- |
