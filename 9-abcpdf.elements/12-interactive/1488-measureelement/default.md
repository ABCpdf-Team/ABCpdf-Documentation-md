# MeasureElement Class

This class represents the measure dictionary. This is definitively detailed in:.

[The ISO PDF Specification, ISO 32000-1:2008 PDF 1.7; Table: 261, page 480.](https://opensource.adobe.com/dc-acrobat-sdk-docs/standards/pdfstandards/pdf/PDF32000_2008.pdf#page=488)

[The ISO PDF Specification, ISO 32000-2:2017 PDF 2.0; Table: 266, page 593.](https://www.iso.org/standard/63534.md)

[Adobe Supplement to the ISO 32000, BaseVersion: 1.7, ExtensionLevel: 3; Table: 8.110, page 49.](http://www.adobe.com/content/dam/Adobe/en/devnet/acrobat/pdfs/adobe_supplement_iso32000.pdf#page=49)

``` System.Object WebSupergoo.ABCpdf13.Elements.Element WebSupergoo.ABCpdf13.Elements.MeasureElement WebSupergoo.ABCpdf13.Elements.GeospatialMeasureElement WebSupergoo.ABCpdf13.Elements.RectilinearMeasureElement ```

## Method Description MeasureElement Create a new MeasureElement. inherited methods... &nbsp;

## Property Description EntryType Represents the "Type" entry of the measure dictionary object. EntrySubtype Represents the "Subtype" entry of the measure dictionary object. inherited properties...