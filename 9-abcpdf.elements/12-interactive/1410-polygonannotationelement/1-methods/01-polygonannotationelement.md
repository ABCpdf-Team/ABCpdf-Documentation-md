...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new PolygonAnnotationElement. |  |  | 
...
| **[C#]** ```csharp PolygonAnnotationElement() PolygonAnnotationElement(Atom atom, IndirectObject host) PolygonAnnotationElement(IndirectObject obj) PolygonAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` PolygonAnnotationElement() PolygonAnnotationElement(atom As Atom, host As IndirectObject) PolygonAnnotationElement(obj As IndirectObject) PolygonAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 1326 in the Annotations example project. [C#] ```csharp PolygonAnnotationElement annot = new PolygonAnnotationElement(AnnotationElement.Object); double[] vertices = Array.ConvertAll(polygon.Split(new char[] { ' ' }), s => double.Parse(s, NumberFormatInfo.InvariantInfo)); annot.EntrySubtype = "Polygon"; annot.EntryVertices = vertices; annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(InteractiveForm.GetBoundingRect(vertices)), annot.Host); annot.EntryC = Array.ConvertAll(borderColor.Split(new char[] { ' ' }), s => double.Parse(s, NumberFormatInfo.InvariantInfo) / 255); annot.EntryIC = Array.ConvertAll(fillColor.Split(new char[] { ' ' }), s => double.Parse(s, NumberFormatInfo.InvariantInfo) / 255); AnnotationElement = annot; ``` |  |  | 
| --- | --- | --- |
