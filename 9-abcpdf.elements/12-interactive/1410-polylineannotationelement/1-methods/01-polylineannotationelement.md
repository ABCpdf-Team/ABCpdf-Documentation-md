...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new PolylineAnnotationElement. |  |  | 
...
| **[C#]** ```csharp PolylineAnnotationElement() PolylineAnnotationElement(Atom atom, IndirectObject host) PolylineAnnotationElement(IndirectObject obj) PolylineAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` PolylineAnnotationElement() PolylineAnnotationElement(atom As Atom, host As IndirectObject) PolylineAnnotationElement(obj As IndirectObject) PolylineAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 1298 in the Annotations example project. [C#] ```csharp PolylineAnnotationElement annot = new PolylineAnnotationElement(AnnotationElement.Object); double[] vertices = Array.ConvertAll(polygon.Split(new char[] { ' ' }), s => double.Parse(s, NumberFormatInfo.InvariantInfo)); annot.EntrySubtype = "PolyLine"; annot.EntryVertices = vertices; annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(InteractiveForm.GetBoundingRect(vertices)), annot.Host); annot.EntryC = Array.ConvertAll(borderColor.Split(new char[] { ' ' }), s => double.Parse(s, NumberFormatInfo.InvariantInfo) / 255); AnnotationElement = annot; ``` |  |  | 
| --- | --- | --- |
