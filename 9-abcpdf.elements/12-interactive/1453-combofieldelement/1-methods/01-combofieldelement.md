...
```csharp
ComboFieldElement()
ComboFieldElement(Atom atom, IndirectObject host)
ComboFieldElement(IndirectObject obj)
ComboFieldElement(Element relation, CreationOptions options)
```
...
```
ComboFieldElement()
ComboFieldElement(atom As Atom, host As IndirectObject)
ComboFieldElement(obj As IndirectObject)
ComboFieldElement(relation As Element, options As CreationOptions)
```
...
```csharp
ComboFieldElement choice = new ComboFieldElement(FieldElement.Object);
if (options != null)
  choice.EntryOpt = new ArrayElementElement>(ArrayAtom.FromStrings(options), Host);
if (selected != null)
