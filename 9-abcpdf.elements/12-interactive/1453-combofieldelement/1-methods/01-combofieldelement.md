...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new ComboFieldElement. |  |  | 
...
| **[C#]** ```csharp ComboFieldElement() ComboFieldElement(Atom atom, IndirectObject host) ComboFieldElement(IndirectObject obj) ComboFieldElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` ComboFieldElement() ComboFieldElement(atom As Atom, host As IndirectObject) ComboFieldElement(obj As IndirectObject) ComboFieldElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 269 in the Annotations example project. [C#] ```csharp ComboFieldElement choice = new ComboFieldElement(FieldElement.Object); if (options != null) choice.EntryOpt = new ArrayElementElement>(ArrayAtom.FromStrings(options), Host); if (selected != null) choice.EntryI = selected; ``` |  |  | 
| --- | --- | --- |
