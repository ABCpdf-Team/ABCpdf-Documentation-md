...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new RubberStampAnnotationElement. |  |  | 
...
| **[C#]** ```csharp RubberStampAnnotationElement() RubberStampAnnotationElement(Atom atom, IndirectObject host) RubberStampAnnotationElement(IndirectObject obj) RubberStampAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` RubberStampAnnotationElement() RubberStampAnnotationElement(atom As Atom, host As IndirectObject) RubberStampAnnotationElement(obj As IndirectObject) RubberStampAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 1370 in the Annotations example project. [C#] ```csharp double aspectRatio; FormXObject formXObject = GenerateAppearanceStream(form.Doc, caption, color, out aspectRatio); XRect xrect = new XRect(rect); xrect.Width = xrect.Height * aspectRatio; RubberStampAnnotationElement annot = new RubberStampAnnotationElement(AnnotationElement.Object); annot.EntrySubtype = "Stamp"; annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(xrect), annot.Host); annot.EntryContents = caption; annot.EntryAP = new AppearanceElement(annot); annot.EntryAP.EntryN = new FormXObjectElement(formXObject); AnnotationElement = annot; ``` |  |  | 
| --- | --- | --- |
