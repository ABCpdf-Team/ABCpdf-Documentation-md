...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new JavaScriptActionElement. |  |  | 
...
| **[C#]** ```csharp JavaScriptActionElement() JavaScriptActionElement(Atom atom, IndirectObject host) JavaScriptActionElement(IndirectObject obj) JavaScriptActionElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` JavaScriptActionElement() JavaScriptActionElement(atom As Atom, host As IndirectObject) JavaScriptActionElement(obj As IndirectObject) JavaScriptActionElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from NameTree.cs line 115 in the Annotations example project. [C#] ```csharp CatalogElement cat = new CatalogElement(doc.ObjectSoup.Catalog); if (cat.EntryNames == null) cat.EntryNames = new NameDictionaryElement(cat); if (cat.EntryNames.EntryJavaScript == null) { cat.EntryNames.EntryJavaScript = new NameTreeNodeElementJavaScriptActionElement>(cat); cat.EntryNames.EntryJavaScript.EntryNames = new ArrayElementElement>(cat); } Root = cat.EntryNames.EntryJavaScript; ``` This code snippet is taken from NameTree.cs line 137 in the Annotations example project. [C#] ```csharp JavaScriptActionElement action = new JavaScriptActionElement(Root); action.EntryType = "Action"; action.EntryS = "JavaScript"; if (!useStream) action.EntryJS = new Element(new StringAtom(script), action.Host); else { StreamObject stream = new StreamObject(action.Host.Soup); stream.SetText(script); if (stream.Length > 64) stream.CompressFlate(); action.EntryJS = new Element(stream); } Add(key, action); ``` |  |  | 
| --- | --- | --- |
