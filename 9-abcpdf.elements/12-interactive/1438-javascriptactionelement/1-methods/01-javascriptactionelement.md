...
```csharp
JavaScriptActionElement()
JavaScriptActionElement(Atom atom, IndirectObject host)
JavaScriptActionElement(IndirectObject obj)
JavaScriptActionElement(Element relation, CreationOptions options)
```
...
```
JavaScriptActionElement()
JavaScriptActionElement(atom As Atom, host As IndirectObject)
JavaScriptActionElement(obj As IndirectObject)
JavaScriptActionElement(relation As Element, options As CreationOptions)
```
...
if (cat.EntryNames.EntryJavaScript == null) {
  cat.EntryNames.EntryJavaScript = new NameTreeNodeElementJavaScriptActionElement>(cat);
  cat.EntryNames.EntryJavaScript.EntryNames = new ArrayElementElement>(cat);
}
Root = cat.EntryNames.EntryJavaScript;
```

This code snippet is taken from NameTree.cs line 137 in the Annotations example project.
