...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new SquareAnnotationElement. |  |  | 
...
| **[C#]** ```csharp SquareAnnotationElement() SquareAnnotationElement(Atom atom, IndirectObject host) SquareAnnotationElement(IndirectObject obj) SquareAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` SquareAnnotationElement() SquareAnnotationElement(atom As Atom, host As IndirectObject) SquareAnnotationElement(obj As IndirectObject) SquareAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 1519 in the Annotations example project. [C#] ```csharp SquareAnnotationElement annot = new SquareAnnotationElement(AnnotationElement.Object); annot.EntrySubtype = "Square"; annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(new XRect(rect)), annot.Host); annot.EntryC = Array.ConvertAll(borderColor.Split(new char[] { ' ' }), s => double.Parse(s, NumberFormatInfo.InvariantInfo) / 255.0); annot.EntryIC = Array.ConvertAll(fillColor.Split(new char[] { ' ' }), s => double.Parse(s, NumberFormatInfo.InvariantInfo) / 255.0); AnnotationElement = annot; ``` |  |  | 
| --- | --- | --- |
