...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new InteractiveFormElement. |  |  | 
...
| **[C#]** ```csharp InteractiveFormElement() InteractiveFormElement(Atom atom, IndirectObject host) InteractiveFormElement(IndirectObject obj) InteractiveFormElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` InteractiveFormElement() InteractiveFormElement(atom As Atom, host As IndirectObject) InteractiveFormElement(obj As IndirectObject) InteractiveFormElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 373 in the Annotations example project. [C#] ```csharp Doc = inDoc; CatalogElement cat = new CatalogElement(Doc.ObjectSoup.Catalog); if (cat.EntryAcroForm == null) { cat.EntryAcroForm = new InteractiveFormElement(cat, CreationOptions.Indirect); cat.EntryAcroForm.EntryFields = new ArrayElementFieldElement>(cat); } else { // convert indirect reference to direct one if (cat.EntryAcroForm.EntryFields.Ref != null) cat.EntryAcroForm.EntryFields = cat.EntryAcroForm.EntryFields; } // add font for text fields IndirectObject times = Doc.ObjectSoup[Doc.AddFont("Times-Roman")]; if (cat.EntryAcroForm.EntryDR == null) cat.EntryAcroForm.EntryDR = new ResourceElement(cat); if (cat.EntryAcroForm.EntryDR.EntryFont == null) cat.EntryAcroForm.EntryDR.EntryFont = new DictElementFontElement>(cat); cat.EntryAcroForm.EntryDR.EntryFont["TimesRoman"] = new FontElement(times); FormElement = cat.EntryAcroForm; ``` |  |  | 
| --- | --- | --- |
