...
InteractiveFormElement(Atom atom, IndirectObject host)
InteractiveFormElement(IndirectObject obj)
InteractiveFormElement(Element relation, CreationOptions options)
```
...
```
InteractiveFormElement()
InteractiveFormElement(atom As Atom, host As IndirectObject)
InteractiveFormElement(obj As IndirectObject)
InteractiveFormElement(relation As Element, options As CreationOptions)
```
...
if (cat.EntryAcroForm == null) {
  cat.EntryAcroForm = new InteractiveFormElement(cat, CreationOptions.Indirect);
  cat.EntryAcroForm.EntryFields = new ArrayElementFieldElement>(cat);
}
...
if (cat.EntryAcroForm.EntryDR == null)
  cat.EntryAcroForm.EntryDR = new ResourceElement(cat);
if (cat.EntryAcroForm.EntryDR.EntryFont == null)
  cat.EntryAcroForm.EntryDR.EntryFont = new DictElementFontElement>(cat);
cat.EntryAcroForm.EntryDR.EntryFont["TimesRoman"] = new FontElement(times);
