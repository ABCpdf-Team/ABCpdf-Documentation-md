...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new TextAnnotationElement. |  |  | 
...
| **[C#]** ```csharp TextAnnotationElement() TextAnnotationElement(Atom atom, IndirectObject host) TextAnnotationElement(IndirectObject obj) TextAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` TextAnnotationElement() TextAnnotationElement(atom As Atom, host As IndirectObject) TextAnnotationElement(obj As IndirectObject) TextAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 1715 in the Annotations example project. [C#] ```csharp TextAnnotationElement annot = new TextAnnotationElement(AnnotationElement.Object); annot.EntrySubtype = "Text"; annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(new XRect(rect1)), annot.Host); annot.EntryName = "Comment"; annot.EntryContents = contents; if (!string.IsNullOrEmpty(rect2)) { PopupAnnotation popup = new PopupAnnotation(form, rect2, AnnotationElement.Object.ID); annot.EntryPopup = (PopUpAnnotationElement)popup.AnnotationElement; } AnnotationElement = annot; ``` |  |  | 
| --- | --- | --- |
