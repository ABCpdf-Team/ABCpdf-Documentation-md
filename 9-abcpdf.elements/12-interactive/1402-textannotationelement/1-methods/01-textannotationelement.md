...
TextAnnotationElement(Atom atom, IndirectObject host)
TextAnnotationElement(IndirectObject obj)
TextAnnotationElement(Element relation, CreationOptions options)
```
...
```
TextAnnotationElement()
TextAnnotationElement(atom As Atom, host As IndirectObject)
TextAnnotationElement(obj As IndirectObject)
TextAnnotationElement(relation As Element, options As CreationOptions)
```
...
```csharp
TextAnnotationElement annot = new TextAnnotationElement(AnnotationElement.Object);
annot.EntrySubtype = "Text";
annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(new XRect(rect1)), annot.Host);
...
if (!string.IsNullOrEmpty(rect2)) {
  ...
  annot.EntryPopup = (PopUpAnnotationElement)popup.AnnotationElement;
}
