...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new CircleAnnotationElement. |  |  | 
...
| **[C#]** ```csharp CircleAnnotationElement() CircleAnnotationElement(Atom atom, IndirectObject host) CircleAnnotationElement(IndirectObject obj) CircleAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` CircleAnnotationElement() CircleAnnotationElement(atom As Atom, host As IndirectObject) CircleAnnotationElement(obj As IndirectObject) CircleAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 90 in the Annotations example project. [C#] ```csharp // The BS entry is not applicable for all Annotation types. However we coerce it into a type which supports it. CircleAnnotationElement element = new CircleAnnotationElement(AnnotationElement.Object); if (element.EntryBS == null) element.EntryBS = new BorderStyleElement(AnnotationElement); return element.EntryBS; ``` |  |  | 
| --- | --- | --- |
