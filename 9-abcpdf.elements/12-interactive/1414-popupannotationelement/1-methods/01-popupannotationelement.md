...
PopUpAnnotationElement(Atom atom, IndirectObject host)
PopUpAnnotationElement(IndirectObject obj)
PopUpAnnotationElement(Element relation, CreationOptions options)
```
...
```
PopUpAnnotationElement()
PopUpAnnotationElement(atom As Atom, host As IndirectObject)
PopUpAnnotationElement(obj As IndirectObject)
PopUpAnnotationElement(relation As Element, options As CreationOptions)
```
...
```csharp
PopUpAnnotationElement annot = new PopUpAnnotationElement(AnnotationElement.Object);
annot.EntrySubtype = "Popup";
annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(new XRect(rect)), annot.Host);
annot.EntryParent = parentID != 0 ? new AnnotationElement(form.Doc.ObjectSoup[parentID]) : null;
