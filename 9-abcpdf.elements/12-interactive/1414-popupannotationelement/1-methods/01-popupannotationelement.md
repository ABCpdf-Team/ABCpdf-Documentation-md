...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new PopUpAnnotationElement. |  |  | 
...
| **[C#]** ```csharp PopUpAnnotationElement() PopUpAnnotationElement(Atom atom, IndirectObject host) PopUpAnnotationElement(IndirectObject obj) PopUpAnnotationElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` PopUpAnnotationElement() PopUpAnnotationElement(atom As Atom, host As IndirectObject) PopUpAnnotationElement(obj As IndirectObject) PopUpAnnotationElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 1688 in the Annotations example project. [C#] ```csharp PopUpAnnotationElement annot = new PopUpAnnotationElement(AnnotationElement.Object); annot.EntrySubtype = "Popup"; annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(new XRect(rect)), annot.Host); annot.EntryParent = parentID != 0 ? new AnnotationElement(form.Doc.ObjectSoup[parentID]) : null; annot.EntryOpen = true; AnnotationElement = annot; ``` |  |  | 
| --- | --- | --- |
