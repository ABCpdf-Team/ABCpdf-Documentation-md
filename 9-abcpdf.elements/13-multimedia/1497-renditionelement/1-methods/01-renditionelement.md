...
```csharp
RenditionElement()
RenditionElement(Atom atom, IndirectObject host)
RenditionElement(IndirectObject obj)
RenditionElement(Element relation, CreationOptions options)
```
...
```
RenditionElement()
RenditionElement(atom As Atom, host As IndirectObject)
RenditionElement(obj As IndirectObject)
RenditionElement(relation As Element, options As CreationOptions)
```
