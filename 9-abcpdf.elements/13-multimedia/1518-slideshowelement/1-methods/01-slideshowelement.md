...
```csharp
SlideshowElement()
SlideshowElement(Atom atom, IndirectObject host)
SlideshowElement(IndirectObject obj)
SlideshowElement(Element relation, CreationOptions options)
```
...
```
SlideshowElement()
SlideshowElement(atom As Atom, host As IndirectObject)
SlideshowElement(obj As IndirectObject)
SlideshowElement(relation As Element, options As CreationOptions)
```
