...
```csharp
TimespanElement()
TimespanElement(Atom atom, IndirectObject host)
TimespanElement(IndirectObject obj)
TimespanElement(Element relation, CreationOptions options)
```
...
```
TimespanElement()
TimespanElement(atom As Atom, host As IndirectObject)
TimespanElement(obj As IndirectObject)
TimespanElement(relation As Element, options As CreationOptions)
```
