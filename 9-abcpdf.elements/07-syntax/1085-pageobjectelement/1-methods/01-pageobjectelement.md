...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new PageObjectElement. |  |  | 
...
| **[C#]** ```csharp PageObjectElement() PageObjectElement(Atom atom, IndirectObject host) PageObjectElement(IndirectObject obj) PageObjectElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` PageObjectElement() PageObjectElement(atom As Atom, host As IndirectObject) PageObjectElement(obj As IndirectObject) PageObjectElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from Annotations.cs line 191 in the Annotations example project. [C#] ```csharp if (Form.Doc.Page == 0) throw new ArgumentException("No page available"); var page = new PageObjectElement(Form.Doc.ObjectSoup[Form.Doc.Page]); AnnotationElement.EntryP = page; if (page.EntryAnnots == null) page.EntryAnnots = new ArrayElementAnnotationElement>(AnnotationElement); page.EntryAnnots.Add(AnnotationElement); ``` |  |  | 
| --- | --- | --- |
