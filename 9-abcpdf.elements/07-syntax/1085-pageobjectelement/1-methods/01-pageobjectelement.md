...
```csharp
PageObjectElement()
PageObjectElement(Atom atom, IndirectObject host)
PageObjectElement(IndirectObject obj)
PageObjectElement(Element relation, CreationOptions options)
```
...
```
PageObjectElement()
PageObjectElement(atom As Atom, host As IndirectObject)
PageObjectElement(obj As IndirectObject)
PageObjectElement(relation As Element, options As CreationOptions)
```
...
var page = new PageObjectElement(Form.Doc.ObjectSoup[Form.Doc.Page]);
AnnotationElement.EntryP = page;
if (page.EntryAnnots == null)
  page.EntryAnnots = new ArrayElementAnnotationElement>(AnnotationElement);
page.EntryAnnots.Add(AnnotationElement);
```
