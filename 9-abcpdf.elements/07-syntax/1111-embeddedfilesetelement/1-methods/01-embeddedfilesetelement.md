...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new EmbeddedFileSetElement. |  |  | 
...
| **[C#]** ```csharp EmbeddedFileSetElement() EmbeddedFileSetElement(Atom atom, IndirectObject host) EmbeddedFileSetElement(IndirectObject obj) EmbeddedFileSetElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` EmbeddedFileSetElement() EmbeddedFileSetElement(atom As Atom, host As IndirectObject) EmbeddedFileSetElement(obj As IndirectObject) EmbeddedFileSetElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from NameTree.cs line 36 in the Annotations example project. [C#] ```csharp FileInfo info = new FileInfo(filePath); EmbeddedFileStreamElement stream = EmbedFileStream(info, soup); CatalogElement cat = new CatalogElement(soup.Catalog); FullFileSpecificationElement file = new FullFileSpecificationElement(cat); file.EntryType = "Filespec"; file.EntryF = info.Name; file.EntryUF = info.Name; file.EntryEF = new EmbeddedFileSetElement(file); file.EntryEF.EntryF = stream; file.EntryDesc = description; return file; ``` |  |  | 
| --- | --- | --- |
