...
```csharp
RectangleElement()
RectangleElement(Atom atom, IndirectObject host)
RectangleElement(IndirectObject obj)
RectangleElement(Element relation, CreationOptions options)
```
...
```
RectangleElement()
RectangleElement(atom As Atom, host As IndirectObject)
RectangleElement(obj As IndirectObject)
RectangleElement(relation As Element, options As CreationOptions)
```
...
This code snippet is taken from Annotations.cs line 106 in the Annotations example project.

[C#]

```csharp
AnnotationElement.EntryRect = new RectangleElement(ArrayAtom.FromXRect(value), AnnotationElement.Host);
```

This code snippet is taken from Annotations.cs line 1209 in the Annotations example project.
...
This code snippet is taken from Annotations.cs line 1546 in the Annotations example project.

[C#]

```csharp
CircleAnnotationElement annot = new CircleAnnotationElement(AnnotationElement.Object);
annot.EntrySubtype = "Circle";
annot.EntryRect = new RectangleElement(ArrayAtom.FromXRect(new XRect(rect)), annot.Host);
