...
| --- | --- | --- | --- | --- |
|  |  |  | 
| Create a new NameDictionaryElement. |  |  | 
...
| **[C#]** ```csharp NameDictionaryElement() NameDictionaryElement(Atom atom, IndirectObject host) NameDictionaryElement(IndirectObject obj) NameDictionaryElement(Element relation, CreationOptions options) ``` [Visual Basic] ``` NameDictionaryElement() NameDictionaryElement(atom As Atom, host As IndirectObject) NameDictionaryElement(obj As IndirectObject) NameDictionaryElement(relation As Element, options As CreationOptions) ``` |  |  | 
| --- | --- | --- |
...
| This code snippet is taken from NameTree.cs line 90 in the Annotations example project. [C#] ```csharp CatalogElement cat = new CatalogElement(doc.ObjectSoup.Catalog); if (cat.EntryNames == null) cat.EntryNames = new NameDictionaryElement(cat); if (cat.EntryNames.EntryEmbeddedFiles == null) { cat.EntryNames.EntryEmbeddedFiles = new NameTreeNodeElementFileSpecificationElement>(cat); cat.EntryNames.EntryEmbeddedFiles.EntryNames = new ArrayElementElement>(cat); } Root = cat.EntryNames.EntryEmbeddedFiles; ``` |  |  | 
| --- | --- | --- |
