...
NameDictionaryElement(Atom atom, IndirectObject host)
NameDictionaryElement(IndirectObject obj)
NameDictionaryElement(Element relation, CreationOptions options)
```
...
```
NameDictionaryElement()
NameDictionaryElement(atom As Atom, host As IndirectObject)
NameDictionaryElement(obj As IndirectObject)
NameDictionaryElement(relation As Element, options As CreationOptions)
```
...
if (cat.EntryNames.EntryEmbeddedFiles == null) {
  cat.EntryNames.EntryEmbeddedFiles = new NameTreeNodeElementFileSpecificationElement>(cat);
  cat.EntryNames.EntryEmbeddedFiles.EntryNames = new ArrayElementElement>(cat);
}
