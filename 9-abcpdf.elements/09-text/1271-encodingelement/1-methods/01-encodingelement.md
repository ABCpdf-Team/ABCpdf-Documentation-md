...
```csharp
EncodingElement()
EncodingElement(Atom atom, IndirectObject host)
EncodingElement(IndirectObject obj)
EncodingElement(Element relation, CreationOptions options)
```
...
```
EncodingElement()
EncodingElement(atom As Atom, host As IndirectObject)
EncodingElement(obj As IndirectObject)
EncodingElement(relation As Element, options As CreationOptions)
```
