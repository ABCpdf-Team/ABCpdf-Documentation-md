...
| The following code embeds the font 'Comic Sans MS' into the document and then adds some text. [C#] ```csharp using var doc = new Doc(); doc.FontSize = 216; string font = "Comic Sans MS"; doc.Font = doc.EmbedFont(font); doc.AddText(font); doc.Save(Server.MapPath("docembedfont.pdf")); ``` [Visual Basic] ```vbnet Using doc As New Doc() doc.FontSize = 216 Dim theFont As String = "Comic Sans MS" doc.Font = doc.EmbedFont(theFont) doc.AddText(theFont) doc.Save(Server.MapPath("docembedfont.pdf")) End Using ``` docembedfont.pdf |  |  | 
| --- | --- | --- |
