...
| **[C#]** ```csharp byte[] GetData() ``` [Visual Basic]`Function GetData() As Byte()` `may throw Exception()` |  |  | 
| --- | --- | --- |
...
| The following code illustrates how one might add text to a PDF and then write it direct to the client browser. This code is an entire ASP.NET page - hello.aspx. [C#] ```csharp ``` [Visual Basic] ```vbnet ``` hello.asp |  |  | 
| --- | --- | --- |
