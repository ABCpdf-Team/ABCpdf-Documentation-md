...
```csharp
static Atom SetItem(Atom atom, string key, Atom val)
static Atom SetItem(Atom atom, int index, Atom val)
```

<span class=language>[Visual Basic]</span>  

```
Shared Function SetItem(atom As Atom, key As String, val As Atom) As Atom
Shared Function SetItem(atom As Atom, index As Integer, val As Atom) As Atom
```
