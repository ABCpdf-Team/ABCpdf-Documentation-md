...
bool Validate(Signature.CertificateCollection certificates) // non-caching
```
...
Function Validate(certificates As Signature.CertificateCollection) As Boolean ' non-caching
```
`may throw Exception()`
...
To be precise, this method updates the properties [Algorithm](../2-properties/algorithm.md), [IsModified](../2-properties/ismodified.md), [IsTimeValid](../2-properties/IsTimeValid.md), [IsTrusted](../2-properties/IsTrusted.md) and [CompliancePades](../2-properties/compliancepades.md). The return value is true only if the [IsModified](../2-properties/ismodified.md) is false, and all of [IsTimeValid,](../2-properties/IsTimeValid.md) [IsSecure](../2-properties/issecure.md) and [IsTrusted](../2-properties/IsTrusted.md) are true.
...
After a document is signed with an approval signature, the document is not necessarily frozen. Most obviously it may need to be updated if another user wishes to add their own signature. Each time the document is updated a new revision is created. The validity of a signature only refers to the revision at which it was signed - the [SigningRevision.](../2-properties/signingrevision.md)So while the signing revision does not affect validity you should be careful to take any subsequent revisions into account when reporting back to the user. Similarly there are some subtleties relating to the [IsTrusted](../2-properties/IsTrusted.md) property which are worth bearing in mind. Hybrid documents may have multiple different visual representations depending on the capabilities of the viewing application, so you may wish to be cautious if your documents contain hybrid sections. For how to detect hybrid sections see the [ObjectSoup.Eof.Load](/6-abcpdf.objects/2-objectsoup.eof/1-methods/01-load.md) method.
...
Signatures' certificates can only be validated by referencing certificates issued by certification authorities. This method allows you to check and validate the status of a signature with reference to a set of such certificates. Additionally, ABCpdf can also use certificates found in the Windows Certificate Store for validation. See [ValidationPolicy](../2-properties/validationpolicy.md) for details.

Except for the non-caching overload, the certificates you provide will be cached at a document level so this function is efficient even when checking multiple signatures within one document. If you do not provide any parameters, this function will use the previously cached certificates to validate the document. Therefore, unless [ValidationPolicy](../2-properties/validationpolicy.md) is set to EntireChainTrust, or certificates have been provided using a previous call to this function, calling the parameterless overload of this function will cause an exception to be thrown to indicate that there are no certificates to validate against.

The overload that takes an [Signature.CertificateCollection](../../signature.certificatecollection/default.md) does not cache the input certificates and does not use any certificates cached with the document. It only uses the certificates provided in the parameter. It allows the use the same set of certificates to validate signatures in multiple documents without re-reading the certificate files.

ABCpdf does not do revocation checks on certificates provided and on certificates embedded in a PDF document unless the [CompliancePades](/6-abcpdf.objects/signature/2-properties/compliancepades.md) is set to something other than None. If you need to do this type of operation, you should use the [GetCertificates](getcertificates.md) function and check the certificates yourself.
...
- [Certified Document Services (CDS)](http://www.adobe.com/security/partners_cds.md) is a trust hierarchy that chains back to the Adobe Root Certification Authority (Adobe Root CA).
- [Adobe Approved Trust List (AATL)](http://www.adobe.com/security/approved-trust-list.md) is an extra list of CA certificates that Adobe Reader may download from Adobe periodically (for Adobe Reader/Acrobat 9 or later).
