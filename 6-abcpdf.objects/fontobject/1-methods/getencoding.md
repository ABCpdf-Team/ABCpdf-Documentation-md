...
```csharp
static Dictionary GetEncoding(LanguageType type, bool vertical)
```

<span class=language>[Visual
            Basic]</span>  

            `Shared Function GetEncoding(type As LanguageType, vertical As Boolean) As Dictionary(Of Char, Char)``may throw Exception()`
